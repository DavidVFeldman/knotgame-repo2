...
| `RequestProject/Closures17.lean` | round 17, T47-T49: no weight bounded below by 1 on (0,1) contracts along every legal move for `1 < lam < 2` (`no_contracting_weight`); the contraction factor `r` of `Contraction.lipschitz_contraction` is attained at the identity, where `P lam id` is affine (`P_id`, `dist_P_id`, `not_lipschitzWith_P_id`); and the density criterion fails at every Pisot parameter, a finite orbit being non-dense in (0,1) (`not_kindDense_of_orb_finite`, `not_denseFrom_half_of_finite`, `not_kindDense_of_isPisot`) |
