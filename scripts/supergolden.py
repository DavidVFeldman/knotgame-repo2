"""Exact supergolden computation: orbit of 1/2, transition tables,
configuration closure, exact maximum, and depth of first attainment d(k).

lam^3 = lam^2 + 1, lam ~ 1.4655712318767682.
...
lo, hi = Fraction(14, 10), Fraction(15, 10)
f = lambda t: t**3 - t**2 - 1
for _ in range(300):
    mid = (lo + hi) / 2
    if f(mid) < 0:
        lo = mid
...
ltr = lambda A, B, C: (A, B + 2, C - 2)
gtg = lambda A, B, C: (A - 2, B - 2, C + 2)
lth = lambda A, B, C: (A, B + 1, C - 1)
gth = lambda A, B, C: (A - 2, B - 1, C + 1)
